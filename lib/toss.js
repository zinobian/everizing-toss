...
class TossClient {
  ...
  async request(method, path, { query, body, needAccount = false } = {}) {
    ...
    const curlArgs = ['-4', '-s', '--max-time', '25', '-X', method, url.toString(), '-H', 'Authorization: Bearer ' + token];
    if (needAccount) curlArgs.push('-H', 'X-Tossinvest-Account: ' + String(this.accountSeq));
    if (body) {
      curlArgs.push('-H', 'Content-Type: application/json', '-d', JSON.stringify(body));
    }
    const { stdout } = await execFileAsync('curl', curlArgs);
    const text = stdout || '';
    const res = { ok: true, status: 200, statusText: 'OK' };
    try {
      const parsedEarly = text ? JSON.parse(text) : null;
      if (parsedEarly && parsedEarly.error) {
        const msg = parsedEarly.error.message || parsedEarly.error.code || 'error';
        throw new Error('토스 API 오류 ' + msg);
      }
    } catch (e) {
      if (String(e.message || '').startsWith('토스 API 오류')) throw e;
    }
    let data;
    try {
  ...
  async createBuyLimitOrder({ symbol, quantity, price, clientOrderId, timeInForce = 'DAY' }) {
    const body = {
      symbol,
      side: 'BUY',
      orderType: 'LIMIT',
    ...
    if (clientOrderId) body.clientOrderId = clientOrderId;
    return this.request('POST', '/api/v1/orders', { body, needAccount: true });
  }
  async cancelOrder(orderId) {
    return this.request('POST', `/api/v1/orders/${orderId}/cancel`, { needAccount: true });
  }
