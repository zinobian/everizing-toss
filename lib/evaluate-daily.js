...
async function runDailyEvaluation({ toss, state, dryRun = true }) {
  ...
  for (const sym of MAIN_SYMBOLS) {
    ...
    if (result.action === 'UPDATE_COUNT') {
      ...
      // 온주 예산통장: 15주 미만이면 익절(규칙1·2·4) 보류, 손절(규칙3)만 허용
      const lotMin = 15;
      const isLot = (sym === 'GDXU' || sym === 'UBOT');
      const isStop = String(result.rule || '').indexOf('RULE3') === 0;
      if (isLot && pos.quantity < lotMin && !isStop) {
        report.logs.push(`[보류] ${sym} ${result.rule}: 보유 ${pos.quantity}주 < ${lotMin}주 (익절 보류)`);
      } else {
        report.signals.push({
          symbol: sym,
          rule: result.rule,
